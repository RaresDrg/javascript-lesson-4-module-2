...
phone numbers in the rows indicate a match. Number of names and phones guaranteed 
to be the same.
*/
 

function printContactsInfo(names, phones) {
  const arayOfNames = names.split(",");
  const arayOfPhones = phones.split(",");

  for (let i = 0; i < arayOfNames.length; i++) {
    const message = `${arayOfNames[i]} - ${arayOfPhones[i]}`;
    console.log(message);
  }
}
