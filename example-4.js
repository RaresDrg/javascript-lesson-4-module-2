...
in the format `<item number> - <item value>`. The numbering of elements 
should start with `1`.
...
function logItems(items) {
  for (let i = 0; i < items.length; i++) {
    ...
    const valueOfItem = items[i];

    const message = `${numberOfItem}: ${valueOfItem}`;
    console.log(message)

    // console.log(`${i + 1}: ${items[i]}`); // *varianta scurta //
  }
}
